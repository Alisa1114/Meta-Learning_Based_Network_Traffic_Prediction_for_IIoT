...
def getsingleGroup(pro_data, group, src_len, tar_len, step, val=False):
    """
    pro_data 全部数据
    group 单组 key
    单组生成序列
    ...
    """
    curent_df = pro_data.loc[
        (pro_data["hostname"] == group[0]) & (pro_data["series"] == group[1])
    ]
...
def get_dataset(
    ...
    train_probility=0.8,
    sample_pro=10000,
    series="memoryallocatedbyproc",
):
    if os.path.exists(
        "train_numpy_samplePro%d_%d_%d.pkl" % (sample_pro, src_len, tar_len)
    ):
        train = pickle.load(
            open(
                "train_numpy_samplePro%d_%d_%d.pkl" % (sample_pro, src_len, tar_len),
        ...
        if os.path.exists(
            "valid_numpy_samplePro{}_{}_{}_{}.pkl".format(
                sample_pro, src_len, tar_len, series
            )
        ):
            valid = pickle.load(
                open(
            ...
            train_device = pickle.load(open("store_device.pkl", "rb",))["train"]
            series_device = []
            for d in train_device:
                if d[1] == series:
                    series_device.append(d)
            
            test_all_sample = []
            for k1, k2 in pro_data.groupby(by=["hostname", "series"]):
                if (k1[1] == series) and (k1 not in series_device):
                    test_all_sample.append(k1)
            
            print("生成测试样本...")
            valid_x, valid_y = [], []
            for id_ in tqdm(test_all_sample):
                x_i, y_i = getsingleGroup(
                    pro_data, id_, src_len, tar_len, step, val=True
                )  # 一组样本
                valid_x.extend(x_i)
                valid_y.extend(y_i)
            with open(
        ...
        return (
            train["X"][: sample_pro * 3],
            train["Y"][: sample_pro * 3],
            valid["X"][: int(sample_pro * 0.1)],
            valid["Y"][: int(sample_pro * 0.1)],
        )
    else:
        pro_data = pd.read_csv(inputdir + "above1900_data.csv")
        all_sample = []
        ...
        with open(
            "train_numpy_samplePro%d_%d_%d.pkl" % (sample_pro, src_len, tar_len), "wb"
        ) as f:
        ...
        with open(
            "valid_numpy_samplePro%d_%d_%d_%s.pkl"
            % (sample_pro, src_len, tar_len, series),
            "wb",
        ) as f:
            pickle.dump({"X": np.array(valid_x), "Y": np.array(valid_y)}, f)

        with open("store_device.pkl", "wb") as f:
            pickle.dump({"train": train_all_sample, "test": test_all_sample}, f)
...
def main(Istest):
    ...
    hidden_size = 512
    embed_size = 7  # 输入X的
    de_size = 24  # 输出的序列长度也就是要预测未来多少个小时的
    ...
    # inputdir='../'
    inputdir = "./"
    train_x, train_y, valid_x, valid_y_1 = get_dataset(
        inputdir, en_size, de_size, series=series
    )  # 拿到所有的序列
    # print(np.isnan(train_x).all())
    # print(np.isnan(train_y).all())
    # print(np.isnan(valid_x).all())
    # print(np.isnan(valid_y_1).all())

    # 数据归一化
    train_x = (train_x - train_x.mean()) / train_x.std()
    train_y = (train_y - train_y.mean()) / train_y.std()
    #    #进行数据归一化处理
    valid_mean, valid_std = valid_x.mean(), valid_x.std()
    ...
    for_rescale = (valid_mean, valid_std, valid_y_1)
    ...
    if path.isfile(shapelets_path):
        ...
        #     all_X, get_label_forshapelets(np.vstack((train_y, valid_y))), 20, 7
        # )
        model.learn_shapelets(train_x, get_label_forshapelets(train_y), 20, 7)
        model.save_shapelets(shapelets_path)
        print("shapelets已保存")
    s = open(
        "Flim_flow2graph_traing_log_shapelets_num%d_%d_%d_%s.txt"
        % (K, en_size, de_size, series),
        "w",
    )
    model.fit(
        for_rescale,
