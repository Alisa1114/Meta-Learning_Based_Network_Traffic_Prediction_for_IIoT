...
class Flow2Graph(nn.Module):
    ...
    def __gat_features__(self, X, train=False):
        __shapelet_distance = shapelet_distance(
            ...
            seg_length=self.seg_length,
            tflag=self.tflag,
            tanh=self.kwargs.get("tanh", False),
            debug=self.debug,
            init=self.init,
            warp=self.warp,
            measurement=self.measurement,
        ...
        if self.sort:
            __shapelet_distance = softmax(
                -1 * np.sort(__shapelet_distance, axis=1), axis=1
            )
        if self.softmax and not self.sort:
            __shapelet_distance = softmax(__shapelet_distance, axis=1)
        if self.append:
            # origin = np.array(
            #     [v[0].reshape(-1) for v in self.shapelets], dtype=np.float
            # ).reshape(1, self.K, -1)
            ...
            origin = np.array(
                [v[0].reshape(-1) for v in self.shapelets], dtype=float
            ).reshape(1, self.K, -1)
            return (
                np.concatenate(
                    (
                        __shapelet_distance,
                        np.tile(origin, (__shapelet_distance.shape[0], 1, 1)),
                    ),
                    axis=2,
                ).astype(float),
                adj_matrix,
            ...
            return __shapelet_distance.astype(float), adj_matrix
        ...
        if self.standard_scale:
            for i in range(self.data_size):
                ...
                Debugger.debug_print(
                    "conduct standard scaling on data-{}, with mean {:.2f} and var {:.2f}".format(
                        i, np.mean(X_scale[0, :, i]), np.std(X_scale[0, :, i])
                    )
                )
        return X_scale
    ...
    def fit(
        ...
        clf_func :获取分类标签的函数
        """
        num_segment, data_size = (
            int(X_scale.shape[1] / self.seg_length),
            X_scale.shape[-1],
        )
        ...
        if reset or self.shapelets is None:
            self.learn_shapelets(
                x=np.vstack((X_scale, valid_x_scale)),
        ...
        if os.path.exists("feauture_%d_%d.plk" % (self.K, de_size / 24)):
            X_savel = pickle.load(
                open("feauture_%d_%d.plk" % (self.K, de_size / 24), "rb")
            )
        ...
        train_dataloader = DataLoader(
            dataset, sampler=train_sampler, batch_size=train_batch_size, num_workers=0
        )
        ...
        try:
            ...
            self.reload(self.clf, output_dir, en_size, tar_len)
            out = self.infer(self.clf, valid_x_scale)
            # 输出预测的f1和error distance
            # results = self.eval(
            #     out, torch.tensor(valid_y, dtype=torch.float32).to(self.device)
            # )
            out = out * for_rescale[1] + for_rescale[0]
            results = self.eval(
                out,
                torch.tensor(for_rescale[2], dtype=torch.float32).to(
                    self.device
                ),
            )
            with open("flow2graph_casestudy.pkl", "wb") as fff:
                pickle.dump(
                    {"Y_label": for_rescale[2], "Y_pre": out.cpu().numpy()}, fff
                )
            ...
            if results["RMSE"] < best_rmse:
                ...
                model_to_save = (
                    self.clf.module if hasattr(self.clf, "module") else self.clf
                )  # Only save the model it-self
                os.makedirs(output_dir, exist_ok=True)
                output_model_file = os.path.join(
                    output_dir,
                    "pytorch_time2graph_gcn_{}_{}.bin".format(en_size, tar_len),
                )
        ...
        for idx in range(epoch):
            ...
            for step, batch in enumerate(train_dataloader):
                ...
                out = self.clf(
                    feat, adj, all_shapelets.to(self.device)
                )  # state_orginal ,b,n,2
                #                print(out.device)
                del batch, feat, adj
                gc.collect()
                ...
                if (step + 1) % display_steps == 0:
                    Debugger.info_print(
                        "At Training:  epoch {} step {} loss {}".format(
                            idx, step + 1, avg_loss
                        )
                    )
                    print(
    ...
    def infer(self, model, valid_x_scale, eval_batch_size=2):
    ...
    def eval(self, predict, Groudth):
        ...
        Groudth = Groudth.cpu().numpy()
        predict = predict[:, 0]
        Groudth = Groudth[:, 0]

        # replace zeros in Groudth with 0.0001, so mape won't be very high
        Groudth_process = np.copy(Groudth)
        Groudth_process[Groudth_process == 0] = 1e-4
        rmse = mean_squared_error(Groudth, predict, squared=False)
        mae = mean_absolute_error(Groudth, predict)
        mape = mean_absolute_percentage_error(Groudth_process, predict)
        results["RMSE"] = rmse
        results["MAE"] = mae
        results["MAPE"] = mape
        return results
    ...
    def reload(self, model, output_dir, en_size, tar_len):
        ...
        load_model_path = os.path.join(
            output_dir, "pytorch_time2graph_gcn_{}_{}.bin".format(en_size, tar_len)
        )
