...
def parameterized_gw_torch(x, y, w, torch_dtype, warp=2):
    ...
    """
    distance = np.sum(
        (x.reshape(x.shape[0], -1, x.shape[1]) - expand_array(y=y, warp=warp)) ** 2,
        axis=1,
    )
    assert not torch.any(torch.isnan(w)), "local: {}".format(w)
    softmin_distance = np.sum(
        softmax(-distance.astype(np.float64)).astype(np.float32) * distance, axis=1
    )
    return torch.sqrt(
        torch.sum(torch.from_numpy(softmin_distance).type(torch_dtype) * torch.abs(w))
    )
...
def parameterized_gdtw_torch(x, y, w, torch_dtype, warp=2):
    ...
    return torch.norm(
        (torch.from_numpy(x).type(torch_dtype) * w.reshape(x.shape[0], -1))[dpath[0]]
        - torch.from_numpy(y[dpath[1]]).type(torch_dtype)
    )


def pattern_distance_torch(
    pattern,
    time_series,
    num_segment,
    seg_length,
    ...
    for k in range(num_segment):
        distance[k] = dist_torch(
            x=pattern, y=time_series[k], w=local_factor, torch_dtype=torch_dtype
        )
    return torch.sum(
        F.softmax(-distance * torch.abs(global_factor), dim=0)
        * (distance * torch.abs(global_factor))
    )


def __shapelet_candidate_loss(
    cand,
    time_series_set,
    label,
    num_segment,
    ...
    **kwargs
    ...
    local_factor_variable = Variable(
        torch.ones(seg_length).type(torch_dtype) / seg_length, requires_grad=True
    )
    global_factor_variable = Variable(
        torch.ones(num_segment).type(torch_dtype) / num_segment, requires_grad=True
    )
    current_loss, loss_queue, cnt, nan_cnt = (
        0.0,
        Queue(max_size=int(num_batch * 0.2)),
        0,
        0,
    )
    ...
    if optimizer == "Adam":
        ...
        raise NotImplementedError(
            "unsupported optimizer {} for time-aware shapelets learning".format(
                optimizer
            )
        )
    ...
    while cnt < max_iters:
        ...
        dataloader = DataLoader(
            dataset=dataset_numpy, batch_size=batch_size, sampler=sampler
        )
        batch_cnt = 0
        for x, y in dataloader:
            ...
            if len(lb_idx[0]) == 0 or sample_flag:
                Debugger.debug_print(
                    "weighted sampling exception, positive {:.2f}/{}".format(
                        np.sum(y) / len(y), len(y)
                    )
                )
                continue
            ...
            for k in range(x.shape[0]):
                dist_tensor[k] = pattern_distance_torch(
                    ...
                    local_factor=local_factor_variable,
                    global_factor=global_factor_variable,
            ...
            if (
                ...
                or torch.any(torch.isnan(local_factor_variable))
                or torch.any(torch.isnan(global_factor_variable))
            ):
                local_factor_variable = Variable(
                    torch.ones(seg_length).type(torch_dtype) / seg_length,
                    requires_grad=True,
                )
                global_factor_variable = Variable(
                    torch.ones(num_segment).type(torch_dtype) / num_segment,
                    requires_grad=True,
                )
                current_loss = 1e5
                nan_cnt += 1
                ...
                Debugger.debug_print(
                    "{:.2f}% steps, loss {:.6f} with {:.6f} and penalty {:.6f}".format(
                        batch_cnt * 100 / num_batch,
                        current_loss,
                        current_main_loss,
                        current_penalty_loss,
                    )
                )
            batch_cnt += 1
        cnt += 1
...
def __shapelet_candidate_loss_factory(
    time_series_set,
    label,
    num_segment,
    seg_length,
...
def learn_time_aware_shapelets(
    time_series_set,
    label,
    K,
    C,
    ...
    **kwargs
    ...
    :return:
    """
    cands = generate_shapelet_candidate(
        time_series_set=time_series_set,
        num_segment=num_segment,
        seg_length=seg_length,
        candidate_size=C,
    ...
    parmap = ParMap(
        ...
        monitor=parallel_monitor(
            msg="learning time-aware shapelets",
            size=len(cands),
