    ...
    def __main__(pid, args, queue):
        ...
        for time_series_segments in args:
            ...
            cand_dist = np.linalg.norm(
                time_series_segments.reshape(size, -1) - center_segment.reshape(1, -1),
                axis=1,
            )
            tmp = []
            for cnt in range(n_candiates):
                ...
                cand_dist[update_idx] += np.linalg.norm(
                    time_series_segments[update_idx].reshape(dims, -1)
                    - time_series_segments[idx].reshape(1, -1),
                    axis=1,
                )
...
def generate_shapelet_candidate(
    time_series_set, num_segment, seg_length, candidate_size, **kwargs
):
    __method, __debug = kwargs.get("candidate_method", "greedy"), kwargs.get(
        "debug", True
    )
    ...
    time_series_set = time_series_set.reshape(
        num_time_series, num_segment, seg_length, -1
    )
    assert (
        candidate_size >= num_segment
    ), "candidate-size {} should be larger " "than n_segments {}".format(
        candidate_size, num_segment
    )
    ...
    if __method == "cluster":
        ...
        raise NotImplementedError(
            "unsupported candidate generating method {}".format(__method)
        )
    parmap = ParMap(
        work=work_func(n_clusters, seg_length),
        monitor=parallel_monitor(
            msg="generate candidate by {}".format(__method),
            size=num_segment,
            debug=__debug,
        ),
        njobs=njobs,
    ...
    Debugger.info_print(
        "totally {} candidates with shape {}".format(len(ret), ret.shape)
    )
    return ret


def __shapelet_distance_factory(
    shapelets, num_segment, seg_length, tflag, init, warp, dist, global_flag=False
):
    def __main__(pid, args, queue):
        ret = []
        for time_series in args:
            ...
            if tflag and global_flag:
                for idx, (pattern, local_factor, global_factor, _) in enumerate(
                    shapelets
                ):
                    for k in range(num_segment):
                        tmp[k, idx] = dist(
                            x=pattern, y=time_series[k], w=local_factor, warp=warp
                        ) * np.abs(global_factor[init + k])
...
def shapelet_distance(
    time_series_set, shapelets, seg_length, tflag, tanh, debug, init, warp, measurement
):
    """
    returns:
        np.array, N x m x K
    ...
    parmap = ParMap(
        ...
        monitor=parallel_monitor(
            msg="shapelet distance", size=num_time_series, debug=debug
        ),
        njobs=NJOBS,
    )
...
def transition_matrix(
    ...
    if percentile is not None:
        ...
        Debugger.debug_print(
            "threshold({}) {}, mean {}".format(
                percentile, dist_threshold, np.mean(sdist)
            ),
            debug=debug,
        )
    else:
        ...
        Debugger.debug_print(
            "threshold {}, mean {}".format(dist_threshold, np.mean(sdist)), debug=debug
        )
    ...
    for tidx in range(num_time_series):
        ...
        Debugger.debug_print(
            "{:.2f}% transition matrix computed...".format(
                float(tidx + 1) * 100 / num_time_series
            ),
            debug=debug,
        )
    Debugger.debug_print(
        "{} edges involved in shapelets graph".format(n_edges), debug=debug
    )
...
def adjacent_matrix(
    sdist, num_time_series, num_segment, num_shapelet, percentile, threshold, debug
):
    ...
    for tidx in range(num_time_series):
        ...
        Debugger.debug_print(
            "{:.2f}% adjacent matrix computed...".format(
                float(tidx + 1) * 100 / num_time_series
            ),
            debug=debug,
        )
    if threshold is None:
        ...
        Debugger.debug_print(
            "threshold({}%): {:.6f}, mean-value: {:.6f}".format(
                percentile, threshold, np.mean(tmat)
            ),
            debug=debug,
        )
    else:
        Debugger.debug_print(
            "threshold: {:.6f}, mean-value {:.6f}".format(threshold, np.mean(tmat)),
            debug=debug,
        )
    ...
    Debugger.debug_print(
        "{:.2f}(std: {:.2f}) edges involved in average for each shapelet graph".format(
            np.mean(num_edges), np.std(num_edges)
        ),
        debug=debug,
    )
...
def graph_embedding(tmat, num_shapelet, embed_size, cache_dir, **deepwalk_paras):
    ...
    for idx in range(tmat.shape[0]):
        ...
        ret.append(
            __embedding2mat(
                fpath=embedding_path, num_vertices=num_shapelet, embed_size=embed_size
            )
        )
    return np.array(ret, dtype=np.float32).reshape(
        tmat.shape[0], num_shapelet, embed_size
    )
