...
__all__ = ["NJOBS", "ParMap", "parallel_monitor"]
    ...
    def run_slices(self, slices):
        ...
        if self.njobs == 1:
            ...
            with contextlib.closing(
                mp.Pool(self.njobs, maxtasksperchild=self.__mtpc)
            ) as pool:
                res = pool.map(
                    func_wrapper,
                    [[dill_work_func, slc, report_queue] for slc in slices],
                )
...
def slice_sample(sample, chunk=None, nslice=None):
    ...
    if chunk is None:
        ...
        if nslice is not None:
            raise RuntimeError(
                "chunk ({}) and slice ({}) should not be specified simultaneously".format(
                    chunk, nslice
                )
            )
    ...
    def monitor(queue):
        ...
        while True:
            ...
            if debug:
                print(
                    "[debug]"
                    + "{} executed by {:.2f}%".format(msg, float(cnt) / size * 100)
                    + "\r",
                    end="",
                )
                sys.stdout.flush()
