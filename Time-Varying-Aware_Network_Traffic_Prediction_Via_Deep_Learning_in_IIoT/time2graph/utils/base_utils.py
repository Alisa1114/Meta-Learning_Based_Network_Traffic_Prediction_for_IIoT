    ...
    def fit(self, x, y):
        ...
        for epoch in range(epochs):
            ...
            for samples, labels in dataloader:
                ...
                i = i + len(samples)
                print(
                    "epoch:{}/{}    iter:{}/{}    loss:{}   r2:{} ".format(
                        epoch + 1, epochs, i, data_len, loss, r2
                    )
                )
...
class myNetwork(nn.Module):
    ...
    def foroneGraph(self, x, adj):
        ...
        h_ = self.gIn(
            x, a1
        )  # 5,num_shapelets,hidden_size   h_[-1]为所有shapelets的表示
...
class ModelUtils(object):
    ...
    def clf_paras(self, balanced):
        ...
        if self.kernel == "lr":
            ...
            intercept_scaling = self.kwargs.get(
                "inter_scale", [pow(5, i) for i in range(-3, 3)]
            )
            ...
            for p1, p2 in itertools.product(c, svm_kernel):
                yield {"C": p1, "kernel": p2, "class_weight": class_weight}
        else:
...
class Debugger(object):
    ...
    def time_print(msg, begin, profiling=False):
        if profiling:
            assert isinstance(begin, type(time.time())), "invalid begin time {}".format(
                begin
            )
...
def convert_string(string, val, cvt_type="float"):
    ...
    try:
        ...
        Debugger.warn_print(
            "invalid convert type {}; use float() by default".format(cvt_type)
        )
        return float(string)
    except ValueError as _:
        Debugger.warn_print(
            "invalid convert value {}; return {} by default".format(string, val)
        )
        return val
